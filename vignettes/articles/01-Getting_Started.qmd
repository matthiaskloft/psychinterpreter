...
Perform a standard exploratory factor analysis with 5 factors:
...
```{r show-names}
interpretation$suggested_names |>
  unlist() |>  data.frame(Suggested_Factor_Name = _) |>
...
- **OpenAI** (API key required): `provider = "openai"`
- **Anthropic Claude** (API key required): `provider = "anthropic"`
