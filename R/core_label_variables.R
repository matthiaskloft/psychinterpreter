...
#' @param labeling_args List or labeling_args object. Labeling-specific configuration.
#'   Created with \code{\link{labeling_args}}. Direct parameters take precedence.
...
label_variables <- function(variable_info,
  ...
  # Extract labeling_args if provided
  if (!is.null(labeling_args)) {
    # Only use config values if direct parameter is at default
    if (label_type == "short" && !is.null(labeling_args$label_type)) {
      label_type <- labeling_args$label_type
    }
    if (is.null(max_words) && !is.null(labeling_args$max_words)) {
      max_words <- labeling_args$max_words
    }
    if (is.null(style_hint) && !is.null(labeling_args$style_hint)) {
      style_hint <- labeling_args$style_hint
    }
