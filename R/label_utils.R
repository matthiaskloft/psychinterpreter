...
#' @param remove_articles Logical. Remove articles
#' @param remove_prepositions Logical. Remove prepositions
#' @param max_chars Integer or NULL. Maximum character length
#' @param abbreviate Logical. Apply abbreviation
#' @param max_words Integer or NULL. Maximum word count
#' @param labeling_args labeling_args object. Configuration (direct params take precedence)
#'
...
reformat_labels <- function(labels,
  ...
  if (is.null(labels$parsed_labels)) {
    cli::cli_abort(
      c(
        "Cannot reformat: no parsed labels stored",
        "i" = "This object may have been created before reformatting support was added"
      )
    )
  }

  # Validate parsed_labels structure
  if (!is.data.frame(labels$parsed_labels)) {
    cli::cli_abort(
      c(
        "Cannot reformat: parsed_labels is not a data.frame",
        "i" = "This object may have been created with an older version"
      )
    )
  }

  # Extract parameters from labeling_args if provided (direct params take precedence)
  if (!is.null(labeling_args)) {
    if (sep == " " && !is.null(labeling_args$sep)) {
      sep <- labeling_args$sep
    }
    if (case == "original" && !is.null(labeling_args$case)) {
      case <- labeling_args$case
    }
    if (!remove_articles && !is.null(labeling_args$remove_articles)) {
      remove_articles <- labeling_args$remove_articles
    }
    if (!remove_prepositions && !is.null(labeling_args$remove_prepositions)) {
      remove_prepositions <- labeling_args$remove_prepositions
    }
    if (is.null(max_chars) && !is.null(labeling_args$max_chars)) {
      max_chars <- labeling_args$max_chars
    }
    if (!abbreviate && !is.null(labeling_args$abbreviate)) {
      abbreviate <- labeling_args$abbreviate
    }
    if (is.null(max_words) && !is.null(labeling_args$max_words)) {
      max_words <- labeling_args$max_words
    }
  }

  # Get unformatted labels from parsed_labels (now a data.frame)
  unformatted_labels <- labels$parsed_labels$label
  ...
  new_labels_df <- data.frame(
    variable = labels$parsed_labels$variable,
  ...
  result <- create_variable_labels(
    ...
    parsed_labels = labels$parsed_labels,
...
#'
#' Export variable labels to CSV or Excel format.
#'
#' @param labels variable_labels object or data frame
#' @param file Character. Output file path
#' @param format Character. "csv" or "xlsx"
#'
#' @return Invisible NULL
#' @export
export_labels <- function(labels, file, format = "csv") {
  ...
  if (format == "csv") {
    ...
    cli::cli_alert_success("Labels exported to {.file {file}}")
  } else if (format == "xlsx") {
    ...
    cli::cli_abort("Format must be 'csv' or 'xlsx'")
  }
