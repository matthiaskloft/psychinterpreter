...
#'
#' @export
plot.fa_interpretation <- function(x, type = "heatmap", ...) {
  ...
  loadings_long <- loadings_df |>
    tidyr::pivot_longer(cols = -variable,
                        names_to = "factor",
                        values_to = "loading") |>
    dplyr::mutate(loading_num = as.numeric(ifelse(loading == "", "0", loading)))
  ...
  if (type == "heatmap") {
    p <- ggplot2::ggplot(loadings_long,
      ...
      ggplot2::labs(
        ...
        x = "Factor",
        y = "Variable",
        caption = "Generated with interpret_fa()"
      )
  } else {
    cli::cli_abort(c(
...
#'
#' @export
create_factor_plot <- function(interpretation_results, plot_type = "heatmap") {
  # Simply call the S3 plot method
  plot.fa_interpretation(interpretation_results, type = plot_type)
}
